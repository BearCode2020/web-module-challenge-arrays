...
function removeFlavorByName(array,flavor_strng){
    array.splice(array.indexOf('Vanilla'), 1);
}
...
function copy(NewArr, array){

    const NewArr = array.map(x => x = x);
    console.log(map1);
}
...
function filterByWord(array,FlavorType){
    // for originalFlavors: loop through each item in the array, log only flavors containting FlavorType in its name
    for (i = 0; i < array.length; i++){
        const arr = [];
        if (array[i] = FlavorType){
            arr.push(array[i]);
        } else {
            console.log('skip');
        }
